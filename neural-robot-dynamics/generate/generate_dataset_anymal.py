...
from generate.trajectory_sampler_anymal import TrajectorySamplerAnymal
...
def collect_dataset(
    ...
    simulation_sampler = TrajectorySamplerAnymal(
                            env,
    ...
    rollouts = \
        simulation_sampler.sample_trajectories_action_mode(
            num_transitions, 
            trajectory_length, 
            passive=passive,
    ...
    if 'actions' in rollouts:
        data_grp.create_dataset(
            name = 'actions',
            data = rollouts['actions'].detach().cpu().numpy()
        )
    data_grp.create_dataset(
        name = 'joint_acts', 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env-name', 
                        type=str, 
                        default='AnymalJointPositionControl', 
                        choices=['AnymalJointPositionControl'],
