...
class TrajectorySamplerPupper(TrajectorySampler):
    """
    Trajectory-mode dataset generator for Stanford Pupper.
    """
    ...
    def sample_initial_states(self, num_envs, initial_states):
        ...
        # 1. UP-AXIS LOGIC: Pupper is Z-up, so we use Identity orientation.
        angle = states_body[:, 3:4] * 0.1 
        axis = torch.nn.functional.normalize(states_body[:, 4:7], p = 2.0, dim = -1)
        
        default_quat = torch.tensor([0.0, 0.0, 0.0, 1.0], device = initial_states.device).unsqueeze(0).expand(num_envs, 4)
        delta_quat = torch_utils.exponential_coord_to_quat(axis * angle)
        states_body[:, 3:7] = torch_utils.quat_mul(default_quat, delta_quat)
        
        # 2. VELOCITY INDEXING: Mapping world velocities to Warp's state vector
        q_dim = self.env.dof_q_per_env
        nu_body = states_body[:, q_dim : q_dim + 3]
        omega_body = states_body[:, q_dim + 3 : q_dim + 6]
        
        omega_world = torch_utils.quat_rotate(states_body[:, 3:7], omega_body)
        ...
        initial_states[:, q_dim : q_dim + 3].copy_(nu_world)
        initial_states[:, q_dim + 3 : q_dim + 6].copy_(omega_world)
    ...
    def sample_trajectories_action_mode(
        ...
        """
        KEEP THIS: This is the engine that runs the simulation and 
        extracts contacts, states, and actions into the dictionary.
        """
        rollout_batches = {
            'gravity_dir': [], 'root_body_q': [], 'states': [],
            'contacts': {
                'contact_normals': [], 'contact_depths': [], 
                'contact_points_0': [], 'contact_points_1': [], 'contact_thicknesses': []
            },
            'actions': [], 'joint_acts': [], 'next_states': []
        }
         # allocate buffers
        states = torch.empty(
            trajectory_length, 
